...
/* display refresh, input device read and animation step period.
 * we do not set LV_DEF_REFR_PERIOD, gets set to a default internal value.
 * But doesn't realy matter:
 * display and animation are refreshed manually on raylib loop 
 * also, display timer is deleted
 * input is set to event mode, so timer is paused anyway */
// #define LV_DEF_REFR_PERIOD  1000
